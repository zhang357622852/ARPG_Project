...
#region 外部接口
    ...
    public static IEnumerator Init()
    {
        ...
        while(true)
        {
            ...
            {
                jsonIsOK = false;
                NIDebug.LogException(ex);
            }

            // 解析json格式失败，重新下载配置文件
            if (! jsonIsOK)
            {
                NIDebug.Log("配置文件格式错误");
                idx++;
            ...
            {
                NIDebug.Log(string.Format("配置文件client_version错误"));
                idx++;
                yield return new WaitForSeconds(1f); // 等待一会重试
                continue;
            }
            ...
            {
                NIDebug.Log(string.Format("配置文件version错误"));
                idx++;
                yield return new WaitForSeconds(1f); // 等待一会重试
                continue;
            }
            ...
            {
                NIDebug.Log(string.Format("配置文件ip错误"));
                idx++;
                yield return new WaitForSeconds(1f); // 等待一会重试
                continue;
            }
            ...
            {
                NIDebug.Log(string.Format("配置文件Port错误"));
                idx++;
                yield return new WaitForSeconds(1f); // 等待一会重试
                continue;
            }
            ...
            {
                NIDebug.Log(string.Format("配置文件client_url错误"));
                idx++;
                yield return new WaitForSeconds(1f); // 等待一会重试
                continue;
            }
            ...
            {
                NIDebug.Log(string.Format("配置文件ab_urls错误"));
                idx++;
                yield return new WaitForSeconds(1f); // 等待一会重试
                continue;
            }
